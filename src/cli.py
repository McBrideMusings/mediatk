...
def scan(path, cwd, until):
    ...
    for index, file in enumerate(result):
        compliance, verbose_compliance = is_video_compliant(file, env_config)
...
def probe(path, cwd, until):
    ...
    for index, file in enumerate(result):
        ...
        output_file = f"Probe{index}_{os.path.basename(file)}.txt"
        output_path = os.path.join(output_dir, output_file)
        save_to_file(video_info, output_path)
        click.echo(f"Saved video probe to {output_path}")

@click.command("process") 
@click.argument('path', type=click.Path(exists=True))
def full_process(path):
    env_config = get_environment_config(path)
    result = find_videos(os.path.abspath(path), 1)
